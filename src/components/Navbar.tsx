...
export default function Navbar() {
  ...
  return <header className="relative z-30 border-b border-zinc-800 bg-[#0a0a0a]">
    ...
    {menuOpen && <nav id="mobile-menu" aria-label="Mobile navigation" className="grid gap-5 border-t border-zinc-800 px-6 py-6 lg:hidden">{links.map(([label, href]) => <Link key={label} href={href} onClick={() => setMenuOpen(false)} className="text-xs tracking-widest hover:text-luxury-gold">{label}</Link>)}</nav>}
